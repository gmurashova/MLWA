...
plt.savefig('./TL_Figures_NK_ARC_FL4/Random_Trained_Image_'+input_runname+'.png', bbox_inches = "tight", transparent = True)
###########################################################################
...
plt.savefig('./TL_Figures_NK_ARC_FL4/Images_Viewed_'+input_runname+'.png', bbox_inches = "tight", transparent = True)
###########################################################################
...
#########################################################################                              
base_model = load_model("/mnt/home/murasho1/ML_Kaggle_TL/k_Models/Histology_IC_adam_SCC_lr-0.001_BS16_E400_7.h5")
...
plot_model(new_model, to_file='./TL_Figures_NK_ARC_FL4/Kaggle_Opt-Adam_ReduceLR_10e-3_model_'+input_runname+'.png')
###########################################################################   
...
for i in range(0,9):
    ...
    im = train_images[i]
    plt.imshow(im)
    plt.savefig('./TL_Figures_NK_ARC_FL4/64x64_New_Kaggle_MMI_ARC_Augmented.png', bbox_inches = "tight")
plt.show()
###########################################################################              
...
history = new_model.fit_generator(gen.flow(train_images, train_labels, batch_size = 16),callbacks = [reduce_lr, csv_logger, tensorboard],validation_data=(test_images, test_labels),steps_per_epoch=len(train_images)//16, validation_steps=len(test_images)//16,shuffle = True, epochs=1000, verbose = 1)
...
print(end - start)
## SAVING THE MODEL########################################################
new_model.save("./TL_Models_NK_ARC_FL4/Histology_IC_adam_SCC_lr-0.001_BS16_E400__NoAug_"+input_runname+".h5")
...
plt.savefig('./TL_Plots_NK_ARC_FL4/Training_and_Testing_Accuracy_and_Loss_Opt-Adam_lr-0.001_NoAug_'+input_runname+'.png', transparent = True, bbox_inches = "tight")
########################################################################### 
# 
