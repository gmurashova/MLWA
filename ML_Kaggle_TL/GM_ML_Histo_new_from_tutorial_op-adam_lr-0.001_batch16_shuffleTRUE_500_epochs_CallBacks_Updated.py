...
plt.savefig('./k_Figures/First_Trained_Images_'+input_runname+'.png', bbox_inches = "tight", transparent = True)
########################################################################### 
...
plt.savefig('./k_Figures/Images_Viewed_'+input_runname+'.png', bbox_inches = "tight", transparent = True)
########################################################################### 
...
plot_model(model, to_file='./k_Figures/Kaggle_Opt-Adam_ReduceLR_10e-3_model_'+input_runname+'.png')
###########################################################################   
...
for i in range(0,9):
    plt.subplot(330+1+i)
    im = np.sum(train_images[i], axis = 2)
    plt.imshow(im)
    plt.savefig('./k_Figures/64x64_Kaggle_Augmented.png', bbox_inches = "tight")
plt.show()
########################################################################### 
...
############### Saving the Model############################## 
model.save("./k_Models/Histology_IC_adam_SCC_lr-0.001_BS16_E400_"+input_runname+".h5")
...
fig.savefig('./k_Plots/Training_and_Testing_Accuracy_and_Loss_Opt:Adam_lr:0.001_'+input_runname+'.png', transparent = True, bbox_inches = "tight")
########################################################################### 
