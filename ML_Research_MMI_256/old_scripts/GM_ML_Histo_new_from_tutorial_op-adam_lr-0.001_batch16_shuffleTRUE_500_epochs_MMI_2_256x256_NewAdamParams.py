...
#
#
input_runname = 'TEST_256'
CSV_filename = './TILES/epoch_TEST'
log_D = './TILES/tensorboard_logTEST'
...
Y = np.load('./TILES_256/MMI_OHE_class_labels.npy')
...
train_images = train_images_a.reshape((288,256,256))
train_labels = np.array(y_train)

test_images_a = np.array(x_val)
test_images = test_images_a.reshape((72,256,256))
...
plt.grid(False)
# plt.show()
plt.savefig('./MMI_Figures/Random_Trained_Image_'+input_runname+'.png', bbox_inches = "tight", transparent = True)
#
...
for i in range(25):
    ...
    if train_labels[i] == 0:
        ...
        plt.xlabel(class_names[0])
# plt.show()
plt.savefig('./MMI_Figures/Images_Viewed_'+input_runname+'.png', bbox_inches = "tight", transparent = True)
...
model = keras.Sequential([
    keras.layers.Conv2D(32,kernel_size = (3,3), input_shape = (256,256,1),activation = tf.nn.relu, padding = "valid"),
...
train_images = train_images.reshape(288,256,256, 1)
print(np.shape(train_images))

test_images = test_images.reshape(72,256,256, 1)
...
############## WITH DATA GENERATOR ##################
# history = model.fit_generator(gen.flow(train_images, train_labels, batch_size = 16),validation_data=(test_images, test_labels),steps_per_epoch=len(train_images)//16, validation_steps=len(test_images)//16,callbacks=[reduce_lr],shuffle = True, epochs=1000, verbose = 1)


############## WITHOUT DATA GENERATOR ##################
history = model.fit_generator(gen.flow(train_images, train_labels, batch_size = 16),callbacks = [reduce_lr],validation_data=(test_images, test_labels),steps_per_epoch=len(train_images)//16, validation_steps=len(test_images)//16,shuffle = True, epochs=500, verbose = 1)
...
## SAVING THE MODEL
model.save("./MMI_Models/Histology_IC_adam_SCC_lr-0.001_BS16_E400_"+input_runname+".h5")
...
# plot_model(model, to_file='./MMI_Figures/Opt-Adam_lr-0.001_model__NoAug_'+input_runname+'.png')
...
# plt.show()
plt.savefig('./MMI_Plots/Training_and_Testing_Accuracy_and_Loss_Opt-Adam_lr-0.001_'+input_runname+'.png', transparent = True, bbox_inches = "tight")
